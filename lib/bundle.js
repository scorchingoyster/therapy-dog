...
Bundle.helpers = {
  item: function(params, hash, content) {
    return {
      type: "item",
      id: "_" + uuid.v4(),
  ...
  file: function(params, hash, content) {
    return {
      type: "file",
      id: "_" + uuid.v4(),
      content: content.body().map(unwrapData)
    }
  },

  link: function(params, hash, content) {
    return {
      type: "link",
  ...
  metadata: function(params, hash, content) {
    return {
      type: "metadata",
      id: "_" + uuid.v4(),
      content: content.body().map(unwrapData)
