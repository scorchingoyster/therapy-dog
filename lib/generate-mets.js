...
function generateDmdSec(mets, form, bundle) {
  collectMetadata(bundle.children).forEach(function(metadata) {
    ...
    var template = form.templates.find(function(t) {
      return t.id === metadata.template;
    });
    
    var arrow = new Arrow(XML, template.template);
    var xml = arrow.evaluate(metadata.data);
