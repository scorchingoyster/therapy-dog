...
// amdSec
// <xmlData>
//        <accessControl xmlns="http://cdr.unc.edu/definitions/acl" published="false"/>
//      </xmlData

function generateAmdSec(mets, bundle) {
  collectMetadata(bundle.children).forEach(function(metadata) {
    if (metadata.kind === "access-control") {
      var amdSec =
        mets
            .element("amdSec");

      var rights = amdSec.element("rightsMD", {ID: metadata.id });
...
function generateItem(parent, node) {
  ...
  node.children.filter(function(child) {
    return child.type === "metadata";
      ...
      metadataIds.push({ key: metadata.kind, value: metadata.id });
  });

  if (metadataIds.length) {
    var desc = [], access = [];

    metadataIds.forEach(function(d) {
      if (d.key === "descriptive") {
        desc.push(d.value);
      }

      if (d.key === "access-control") {
        access.push(d.value);
      }
    });

    if (desc.length) div.attribute("DMDID", desc.join(" "));
    if (access.length) div.attribute("AMDID", access.join(" "));
  }
}
