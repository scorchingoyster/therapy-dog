...
const registerDefaultHelpers = require('./helpers').registerDefaultHelpers;
...
function evaluateProgram(environment, context, node) {
  ...
  node.body.forEach(function(child) {
    let results = evaluateNode(environment, context, child);
...
class Arrow {
  ...
  evaluate(input) {
    let result = evaluateNode(this.environment, new Context(input), this.program);
  ...
  static helper(func, options) {
    options = options === undefined ? { raw: false } : options;
    let raw = options.raw === undefined ? false : options.raw;
